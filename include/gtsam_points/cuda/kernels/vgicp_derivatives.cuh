...
struct vgicp_derivatives_kernel {
  vgicp_derivatives_kernel(const Eigen::Isometry3f *linearization_point_ptr,
      ...
      : linearization_point_ptr(linearization_point_ptr),
        ...
        voxel_covs_ptr(voxelmap.voxel_covs), source_means_ptr(source_means),
        source_covs_ptr(source_covs) {}

  __device__ LinearizedSystem6
  operator()(const thrust::pair<int, int> &source_target_correspondence) const {
    ...
    // const Eigen::Matrix3f &cov_A = 1e4 * source_covs_ptr[source_idx];
    // Eigen::Matrix3f cov_A = Eigen::Matrix3f::Zero();
    // cov_A.diagonal() << 1,1,1;
    ...
    // const Eigen::Matrix3f &cov_B = 1e4 * voxel_covs_ptr[target_idx];
    // Eigen::Matrix3f cov_B = Eigen::Matrix3f::Zero();
    // cov_B.diagonal() << 1,1,1;
    ...
    Eigen::Matrix<float, 6, 3> J_target_RCR_inv =
        J_target.transpose() * RCR_inv;
    Eigen::Matrix<float, 6, 3> J_source_RCR_inv =
        J_source.transpose() * RCR_inv;
...
struct vgicp_error_kernel {
  vgicp_error_kernel(const Eigen::Isometry3f *linearization_point_ptr,
