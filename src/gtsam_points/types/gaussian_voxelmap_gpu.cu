...
struct voxel_coord_kernel {
  ...
  __host__ __device__ Eigen::Vector3i
  operator()(const Eigen::Vector3f &x) const {
...
struct voxel_bucket_assignment_kernel {
  voxel_bucket_assignment_kernel(const VoxelMapInfo *voxelmap_info,
                                 const Eigen::Vector3i *point_coords,
                                 thrust::pair<int, int> *index_buckets,
                                 int *voxels_failures)
      : voxelmap_info_ptr(voxelmap_info), point_coords_ptr(point_coords),
        index_buckets_ptr(index_buckets), voxels_failures_ptr(voxels_failures) {
  }
  ...
  __device__ void operator()(int point_index) const {
    ...
    for (int i = 0; i < info.max_bucket_scan_count; i++) {
      ...
      thrust::pair<int, int> *index_bucket =
          thrust::raw_pointer_cast(index_buckets_ptr) + bucket_index;

      int old = atomicCAS(&index_bucket->first, -1, point_index);
      if (old < 0) {
        index_bucket->second =
            atomicAdd(thrust::raw_pointer_cast(voxels_failures_ptr), 1);
        return;
      }
...
struct voxel_coord_select_kernel {
  voxel_coord_select_kernel(const Eigen::Vector3i *point_coords)
      : point_coords_ptr(point_coords) {}

  __device__ VoxelBucket
  operator()(const thrust::pair<int, int> &index_bucket) const {
    if (index_bucket.first < 0) {
      return {Eigen::Vector3i(0, 0, 0), -1};
    }

    return {thrust::raw_pointer_cast(point_coords_ptr)[index_bucket.first],
            index_bucket.second};
  }
...
struct accumulate_points_kernel {
  accumulate_points_kernel(const VoxelMapInfo *voxelmap_info_ptr,
                           const VoxelBucket *buckets, int *num_points,
                           Eigen::Vector3f *voxel_means,
                           Eigen::Matrix3f *voxel_covs)
      : voxelmap_info_ptr(voxelmap_info_ptr), buckets_ptr(buckets),
        num_points_ptr(num_points), voxel_means_ptr(voxel_means),
        voxel_covs_ptr(voxel_covs) {}

  __device__ void operator()(
      const thrust::tuple<Eigen::Vector3f, Eigen::Matrix3f> &input) const {
    const auto &info = *thrust::raw_pointer_cast(voxelmap_info_ptr);
    ...
    for (int i = 0; i < info.max_bucket_scan_count; i++) {
      ...
      if (equal(bucket.first, coord)) {
        int &num_points =
            thrust::raw_pointer_cast(num_points_ptr)[bucket.second];
        Eigen::Vector3f &voxel_mean =
            thrust::raw_pointer_cast(voxel_means_ptr)[bucket.second];
        Eigen::Matrix3f &voxel_cov =
            thrust::raw_pointer_cast(voxel_covs_ptr)[bucket.second];
...
struct finalize_voxels_kernel {
  finalize_voxels_kernel(int *num_points, Eigen::Vector3f *voxel_means,
                         Eigen::Matrix3f *voxel_covs)
      : num_points_ptr(num_points), voxel_means_ptr(voxel_means),
        voxel_covs_ptr(voxel_covs) {}
...
GaussianVoxelMapGPU::GaussianVoxelMapGPU(float resolution, int init_num_buckets,
                                         int max_bucket_scan_count,
                                         double target_points_drop_rate,
                                         CUstream_st *stream)
    : stream(stream), init_num_buckets(init_num_buckets),
  ...
  check_error << cudaMallocAsync(&voxelmap_info_ptr, sizeof(VoxelMapInfo),
                                 stream);
#else
  check_error << cudaMalloc(&voxelmap_info_ptr, sizeof(VoxelMapInfo));
#endif
#endif
  check_error << cudaMemcpyAsync(voxelmap_info_ptr, &voxelmap_info,
                                 sizeof(VoxelMapInfo), cudaMemcpyHostToDevice,
                                 stream);
...
#else
  check_error << cudaFree(voxelmap_info_ptr);
  check_error << cudaFree(buckets);
  check_error << cudaFree(num_points);
  check_error << cudaFree(voxel_means);
  check_error << cudaFree(voxel_covs);
#endif
#endif
}
...
void GaussianVoxelMapGPU::insert(const PointCloud &frame) {
...
#if CUDAToolkit_VERSION_MAJOR == 11
#if CUDAToolkit_VERSION_MINOR >= 4
  check_error << cudaMallocAsync(
      &num_points, sizeof(int) * voxelmap_info.num_voxels, stream);
  check_error << cudaMallocAsync(
      &voxel_means, sizeof(Eigen::Vector3f) * voxelmap_info.num_voxels, stream);
  check_error << cudaMallocAsync(
      &voxel_covs, sizeof(Eigen::Matrix3f) * voxelmap_info.num_voxels, stream);
#else
  check_error << cudaMalloc(&num_points,
                            sizeof(int) * voxelmap_info.num_voxels);
  check_error << cudaMalloc(&voxel_means,
                            sizeof(Eigen::Vector3f) * voxelmap_info.num_voxels);
  check_error << cudaMalloc(&voxel_covs,
                            sizeof(Eigen::Matrix3f) * voxelmap_info.num_voxels);
#endif
#endif

  check_error << cudaMemsetAsync(
      num_points, 0, sizeof(int) * voxelmap_info.num_voxels, stream);
  check_error << cudaMemsetAsync(
      voxel_means, 0, sizeof(Eigen::Vector3f) * voxelmap_info.num_voxels,
      stream);
  check_error << cudaMemsetAsync(
      voxel_covs, 0, sizeof(Eigen::Matrix3f) * voxelmap_info.num_voxels,
      stream);
  ...
  thrust::for_each(
      ...
      thrust::make_zip_iterator(thrust::make_tuple(points_ptr + frame.size(),
                                                   covs_ptr + frame.size())),
      accumulate_points_kernel(voxelmap_info_ptr, buckets, num_points,
                               voxel_means, voxel_covs));

  thrust::for_each(thrust::cuda_cub::execute_on_stream(stream),
...
void GaussianVoxelMapGPU::create_bucket_table(cudaStream_t stream,
...
#if CUDAToolkit_VERSION_MINOR >= 4
  check_error << cudaMallocAsync(
      &coords, sizeof(Eigen::Vector3i) * frame.size(), stream);
#else
  check_error << cudaMalloc(&coords, sizeof(Eigen::Vector3i) * frame.size());
#endif
#endif
  thrust::transform(
      thrust::cuda_cub::execute_on_stream(stream),
      ...
      coords, voxel_coord_kernel(voxelmap_info_ptr));
...
#else
  check_error << cudaMalloc(&voxels_failures, sizeof(int) * 2);
#endif
#endif
  check_error << cudaMemsetAsync(voxels_failures, 0, sizeof(int) * 2, stream);

  for (int num_buckets = init_num_buckets; init_num_buckets * 4;
       num_buckets *= 2) {
    ...
    check_error << cudaMallocAsync(
        &index_buckets, sizeof(thrust::pair<int, int>) * num_buckets, stream);
#else
    check_error << cudaFree(index_buckets);
    check_error << cudaMalloc(&index_buckets,
                              sizeof(thrust::pair<int, int>) * num_buckets);
#endif
#endif
    check_error << cudaMemsetAsync(index_buckets, -1,
                                   sizeof(thrust::pair<int, int>) * num_buckets,
                                   stream);
    check_error << cudaMemsetAsync(voxels_failures, 0, sizeof(int) * 2, stream);
    ...
    thrust::for_each(thrust::cuda_cub::execute_on_stream(stream),
                     ...
                     voxel_bucket_assignment_kernel(voxelmap_info_ptr, coords,
                                                    index_buckets,
                                                    voxels_failures));

    std::array<int, 2> h_voxels_failures;
    check_error << cudaMemcpyAsync(h_voxels_failures.data(), voxels_failures,
                                   sizeof(int) * 2, cudaMemcpyDeviceToHost,
                                   stream);
    check_error << cudaStreamSynchronize(stream);

    if (h_voxels_failures[1] == 0 ||
        static_cast<double>(h_voxels_failures[1]) / frame.size() <=
            target_points_drop_rate) {
  ...
  check_error << cudaMallocAsync(
      &buckets, sizeof(VoxelBucket) * voxelmap_info.num_buckets, stream);
#else
  check_error << cudaFree(buckets);
  check_error << cudaMalloc(&buckets,
                            sizeof(VoxelBucket) * voxelmap_info.num_buckets);
#endif
#endif
  thrust::transform(thrust::cuda_cub::execute_on_stream(stream),
                    thrust::device_ptr<thrust::pair<int, int>>(index_buckets),
                    thrust::device_ptr<thrust::pair<int, int>>(index_buckets) +
                        voxelmap_info.num_buckets,
  ...
  check_error << cudaFreeAsync(index_buckets, stream);
#else
  check_error << cudaFree(coords);
  check_error << cudaFree(voxels_failures);
  check_error << cudaFree(index_buckets);
#endif
#endif
}
...
download_voxel_means(const GaussianVoxelMapGPU &voxelmap, CUstream_st *stream) {
  std::vector<Eigen::Vector3f> means(voxelmap.voxelmap_info.num_voxels);
  check_error << cudaMemcpyAsync(means.data(), voxelmap.voxel_means,
                                 sizeof(Eigen::Vector3f) *
                                     voxelmap.voxelmap_info.num_voxels,
                                 cudaMemcpyDeviceToHost, stream);
  return means;
}

std::vector<Eigen::Matrix3f>
download_voxel_covs(const GaussianVoxelMapGPU &voxelmap, CUstream_st *stream) {
  std::vector<Eigen::Matrix3f> covs(voxelmap.voxelmap_info.num_voxels);
  check_error << cudaMemcpyAsync(covs.data(), voxelmap.voxel_covs,
